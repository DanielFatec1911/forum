...
<div class="create-post-container">
    ...
    <form action="{{ route('updateCategory', [$category->id]) }}" method="POST" class="create-post-form">
        <h2 class="create-post-title">Tag #1</h2>
        @csrf
        <div class="form-group">
            <label for="title" class="form-label">Titulo da Categoria:</label>
            <input type="text" id="title" name="title" class="form-input" placeholder="{{'$category->title' }}" value="{{'$category->title' }}" required>
            @error('title')
            <span>{{ $message }}</span>
            @enderror
            <label for="description" class="form-label">Descrição da Categoria:</label>
            <input type="text" id="description" name="description" class="form-input" value="{{ '$category->description' }}" placeholder="{{'$category->description'}}" value="{{'$category->title' }}" required>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                Você tem certeza que deseja excluir esse Tag?
            </div>
